-- Fix Outside Figure Metadata - Simple version without emojis
-- Compatible with WIN1252 encoding

-- Step 1: Check current state (without area_m2 to avoid column errors)
SELECT 
  id,
  stand,
  designation,
...
-- Step 2: Generate metadata inline (bypassing function)
DO $$
DECLARE
...
BEGIN
  ...
  IF parcel_record.id IS NULL THEN
    RAISE NOTICE 'ERROR: No Outside Figure parcel found for project 5';
    RETURN;
  END IF;
