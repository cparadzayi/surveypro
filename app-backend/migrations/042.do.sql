-- Migration 042: Fix migrate_surveyor_to_schema function
-- Issue: Function tries to copy 'metadata' column that doesn't exist in public.survey_projects

-- Drop the old function
DROP FUNCTION IF EXISTS migrate_surveyor_to_schema(INTEGER);

-- Recreate with correct columns matching actual table structure
...
BEGIN
  ...
  -- Migrate survey_projects (WITHOUT metadata column)
  EXECUTE format('
...
COMMENT ON FUNCTION migrate_surveyor_to_schema IS 'Migrates surveyor data from public schema to surveyor-specific schema (fixed column mismatch)';

-- Success message
SELECT 'Migration 042 completed: Fixed migrate_surveyor_to_schema function' AS status;
