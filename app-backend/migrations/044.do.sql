-- Migration 044: Fix migrate_surveyor_to_schema - remove project_id column
...
BEGIN
  ...
  -- Migrate survey_projects (WITHOUT project_id - doesn't exist in surveyor schema)
  EXECUTE format('
    INSERT INTO %I.survey_projects 
      (id, name, client_name, district, survey_type, survey_date, 
    ...
    SELECT id, name, client_name, district, survey_type, survey_date,
           instruments, designation, status, created_at, updated_at, central_meridian,
           working_directory, surveyor_profile_id, supervising_surveyor_id,
           workflow_state, last_used, stand_reference, township
  ...
  RETURN format('✅ Migrated surveyor %s (ID: %s) to schema %s
📊 Summary:
  - Projects: %s
  - Coordinate Points: %s
  - Land Parcels: %s
...
COMMENT ON FUNCTION migrate_surveyor_to_schema IS 'Migrates surveyor data from public schema to surveyor-specific schema (with stats)';

SELECT 'Migration 044 completed: Removed project_id, added migration stats' AS status;
