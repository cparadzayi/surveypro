-- Migration 045: Fix migrate_surveyor_to_schema - only copy columns that exist in surveyor schema
...
BEGIN
  ...
  -- Migrate survey_projects - ONLY columns that exist in BOTH schemas
  -- Surveyor schema has: id, name, client_name, survey_type, survey_date, district, 
  --                      central_meridian, working_directory, status, metadata, created_at, updated_at
  EXECUTE format('
...
SELECT '✅ Migration 045 completed: Fixed column mismatch (only copying columns that exist in both schemas)' AS status;
