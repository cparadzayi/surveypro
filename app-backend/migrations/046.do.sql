-- Migration 046: Fix coordinate_points migration - use 'z' not 'elevation'
...
BEGIN
  ...
  -- Surveyor schema has: id, project_id, name, y, x, z (NOT elevation!), description, 
  --                      fp, status, point_type, date_surveyed, geom, metadata, created_at, updated_at
  EXECUTE format('
    INSERT INTO %I.coordinate_points 
      (id, project_id, name, y, x, z, description, geom, created_at, updated_at)
    SELECT cp.id, cp.project_id, cp.name, cp.y, cp.x, cp.elevation, 
           cp.description, cp.geom, cp.created_at, cp.updated_at
  ...
  -- Surveyor schema has: id, project_id, stand, designation, owner, title_deed, survey_date,
...
COMMENT ON FUNCTION migrate_surveyor_to_schema IS 'Migrates surveyor data from public to surveyor-specific schema';

SELECT '✅ Migration 046: Fixed coordinate_points (use z column not elevation)' AS status;
