-- Migration 048: Fix land_parcels migration - use actual column names
...
BEGIN
  ...
  -- Surveyor schema has: id, project_id, stand, designation, area_m2, area_ha, 
  --                      closure_error_m (NOT closure_error!), geom, created_at, updated_at
  -- Map: public.closure_error -> surveyor.closure_error_m
  EXECUTE format('
...
COMMENT ON FUNCTION migrate_surveyor_to_schema IS 'Migrates surveyor data from public to surveyor schema';

SELECT '✅ Migration 048: Fixed land_parcels (closure_error → closure_error_m)' AS status;
