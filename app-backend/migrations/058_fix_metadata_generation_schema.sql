-- Migration 058: Fix metadata generation to work with multi-schema system
-- Removes hardcoded schema references and makes functions schema-agnostic

-- Drop existing functions
DROP FUNCTION IF EXISTS generate_parcel_metadata(INTEGER);
DROP FUNCTION IF EXISTS update_parcels_with_missing_metadata(INTEGER);
DROP FUNCTION IF EXISTS trigger_generate_parcel_metadata();
...
BEGIN
  ...
  FOR i IN 1..vertex_count LOOP
    ...
    -- ST_Azimuth returns radians from north (0° = North, clockwise)
    -- Zimbabwe uses south-oriented (0° = South, clockwise)
    -- Conversion: Add 180° to convert North → South orientation
...
'Generates traverse metadata (edges with south-oriented bearings, points) from parcel geometry. Schema-agnostic - uses current search_path.';
