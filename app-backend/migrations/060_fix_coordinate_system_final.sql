...
-- ============================================================================
-- DROP VIEWS THAT DEPEND ON GEOMETRY COLUMNS
-- ============================================================================
...
-- ============================================================================
-- TRANSFORM LAND PARCELS (WITH GENERATED COLUMNS)
-- ============================================================================

-- Drop existing spatial index
DROP INDEX IF EXISTS land_parcels_geom_idx;

-- Drop generated columns that depend on geom
...
-- ============================================================================
-- RECREATE VIEWS WITH NEW SRID
-- ============================================================================

-- View with additional computed attributes for land parcels
...
-- Count records transformed and verify SRID
DO $$
DECLARE
  coord_count INTEGER;
...
BEGIN
  ...
  RAISE NOTICE 'Generated columns (area_m2, area_ha, perimeter_m) recreated';
END $$;
...
BEGIN
  ...
  IF parcel_count > 0 THEN
    RAISE NOTICE 'Sample parcel areas (first 3):';
    RAISE NOTICE '   Format: Stand | Area (m2) | Area (ha) | Perimeter (m)';
    RAISE NOTICE '   --------------------------------------------------------';
    ...
    FOR rec IN 
      ...
      LIMIT 3
    LOOP
      RAISE NOTICE '   % | % m2 | % ha | % m', 
        rec.stand, rec.area_m2, rec.area_ha, rec.perimeter_m;
    END LOOP;
...
BEGIN
  ...
  RAISE NOTICE '   1. Restart backend server (code already updated)';
  RAISE NOTICE '   2. Test in QGIS 3.44:';
  RAISE NOTICE '      - Export coordinates to PostGIS';
  RAISE NOTICE '      - Add layers (should auto-detect EPSG:2053)';
  RAISE NOTICE '      - Add Google Satellite layer';
  RAISE NOTICE '      - Verify perfect overlay, north = UP';
  RAISE NOTICE '   3. After verification, drop backup tables:';
