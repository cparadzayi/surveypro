-- Migration 078: Add township column to survey_projects
-- Purpose: Add township field to support subdivision survey metadata
-- Date: 2025-12-29

-- Add township column to all surveyor schemas
DO $$
DECLARE
  schema_rec RECORD;
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Added township column to schema: %', schema_rec.schema_name;
  END LOOP;
END $$;

-- Update the create_surveyor_schema function to include township in new schemas
...
COMMENT ON FUNCTION create_surveyor_schema IS 'Creates a complete surveyor schema with all necessary tables and permissions (updated with township field)';
