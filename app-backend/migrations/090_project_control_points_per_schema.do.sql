-- Migration 090: give every surveyor schema its own project_control_points
--
-- Migration 062 created this table because saving control points was returning
-- 500. It patched the schemas that existed at the time, dropped its helper, and
-- left create_surveyor_schema() alone -- so the generator still omitted the
-- table and every surveyor created afterwards was born with the same fault.
-- Six of ten schemas were missing it again.
--
-- The failure is worse than a 500. With no table in the schema, search_path
-- falls through to public.project_control_points, whose foreign key points at
-- public.survey_projects. A write for a project that lives in the surveyor's
-- schema is rejected:
--
--   insert or update on table "project_control_points" violates foreign key
--   constraint "project_control_points_project_id_fkey"
--   DETAIL: Key (project_id)=(20) is not present in table "survey_projects".
--
-- and a read succeeds against the wrong tenant's rows whenever the project ids
-- happen to collide, which for four surveyors they did.
--
-- So this migration does both halves: it backfills the schemas that lack the
-- table, and it teaches create_surveyor_schema() to create it. The helper is
-- kept, not dropped, so the backfill and the generator share one definition and
-- cannot drift apart again.
--
-- Idempotent and non-destructive: creates tables and redefines a function,
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.add_project_control_points_to_schema(p_schema_name VARCHAR)
RETURNS VOID AS $$
BEGIN
  EXECUTE format('
...
COMMENT ON FUNCTION public.add_project_control_points_to_schema(VARCHAR) IS
  'Creates a schema''s own project_control_points table. Called by create_surveyor_schema() and by the 090 backfill. Do not drop: migration 062 dropped its equivalent, and the generator then drifted for six schemas.';
    ...
    WHERE t.table_name = 'survey_projects'
      ...
      AND NOT EXISTS (
        ...
        WHERE x.table_schema = t.table_schema
          AND x.table_name = 'project_control_points'
      )
  LOOP
    PERFORM public.add_project_control_points_to_schema(v_schema_name);
    v_count := v_count + 1;
    RAISE NOTICE 'Added project_control_points to schema: %', v_schema_name;
  END LOOP;

  RAISE NOTICE 'Migration 090: repaired % schema(s)', v_count;
END $$;

-- ---------------------------------------------------------------------------
-- The generator, so new surveyors are not born with the same fault. This is the
-- live definition of create_surveyor_schema() with one PERFORM added before the
-- grants block; everything else is byte-for-byte what was already deployed.
-- ---------------------------------------------------------------------------
