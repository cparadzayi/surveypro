...
export default async function surveyPlanPreviewRoutes(fastify, options) {
  ...
  /**
   * GET /api/survey-plan/scales/:projectId
   * Get available scales for a project
   */
  fastify.get('/scales/:projectId', async (request, reply) => {
    const { projectId } = request.params
    const { areaType } = request.query
    
    try {
      // Get surveyor schema for this project
      const schemaResult = await fastify.pg.query(
        `SELECT sp.surveyor_profile_id, spr.schema_name
      ...
      // Fetch coordinate points and parcels from surveyor schema
      // FIX: ST_Y returns Y (Westing), ST_X returns X (Southing)
      const pointsResult = await fastify.pg.query(
        `SELECT name, ST_Y(geom) as y, ST_X(geom) as x 
         FROM ${schemaName}.coordinate_points 
         WHERE project_id = $1`,
        [projectId]
      )
      
      const parcelsResult = await fastify.pg.query(
        `SELECT stand, area_m2 FROM ${schemaName}.land_parcels WHERE project_id = $1`,
        [projectId]
      )
      
      if (pointsResult.rows.length === 0 || parcelsResult.rows.length === 0) {
        return reply.code(404).send({ error: 'Project data not found' })
      }
      
      // Analyze and get scale recommendations
      const analysis = analyzeSurvey(
        pointsResult.rows,
        parcelsResult.rows.map(p => ({ ...p, area_m2: parseFloat(p.area_m2) }))
      )
      
      const scaleResult = determineOptimalScale(analysis, areaType || 'urban')
      
      return {
        recommended: scaleResult.recommended,
        alternatives: scaleResult.alternatives,
        reasoning: scaleResult.reasoning,
        requiresMultiSheet: scaleResult.requiresMultiSheet
      }
      
    } catch (error) {
      fastify.log.error(error)
      return reply.code(500).send({ 
        error: 'Failed to get scales',
        message: error.message 
      })
    }
  })
