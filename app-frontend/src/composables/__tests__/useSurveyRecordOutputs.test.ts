...
describe('saveSurveyRecordSections', () => {
  ...
  it('writes all four sections to their folders with fixed names and overwrite', async () => {
    (saveDocument as any).mockResolvedValue({ success: true, filePath: 'ok' });
    const res = await saveSurveyRecordSections({
      workingDirectory: 'C:/proj', sections,
    });
    expect(saveDocument).toHaveBeenCalledTimes(4);
    const calls = (saveDocument as any).mock.calls.map((c: any[]) => c[0]);
    expect(calls).toEqual(expect.arrayContaining([
      expect.objectContaining({ documentType: 'field-book', fileName: 'FieldBook.pdf', overwrite: true }),
      expect.objectContaining({ documentType: 'coordinate-list', fileName: 'CoordinateList.pdf', overwrite: true }),
      expect.objectContaining({ documentType: 'calculations-part1', fileName: 'Calculations.pdf', overwrite: true }),
      expect.objectContaining({ documentType: 'areas-consistency', fileName: 'AreasAndConsistency.pdf', overwrite: true }),
    ]));
