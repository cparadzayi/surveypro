<template>
  <ModuleScaffold title="Conversions" description="Coordinate & unit transformation utilities." :breadcrumbs="[{label:'Conversions'}]">
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
      <RouterLink v-for="s in submenus" :key="s.slug" :to="`/modules/conversions/${s.slug}`" class="p-3 rounded border bg-white shadow-sm hover:bg-gray-50 text-xs">
...
const module = computed(()=> store.getBySlug('conversions'))
