        ...
        {showSidePanel && calculatedArea && (
          <div className="w-1/3 border-l bg-gray-50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              {/* Professional Survey Calculation Table */}
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <div className="text-center mb-4">
                  <h5 className="font-bold text-gray-900 text-lg border-b-2 border-gray-800 pb-2 inline-block">
                    Stand/Erf Number = {calculatedArea.corners[0]?.point || 'SURVEY'}
                  </h5>
                </div>
                
                {/* Professional Survey Table */}
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse border border-gray-800 font-mono text-sm">
                    <thead>
                      <tr className="bg-gray-100">
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">Bearing</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">Distance</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">Point</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">Y (metres)</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">X (metres)</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">ΔY</th>
                        <th className="border border-gray-800 px-2 py-1 text-center font-bold">ΔX</th>
                      </tr>
                    </thead>
                    <tbody>
                      {calculatedArea.bearingsDistances.map((bd: any, index: number) => {
                        const currentCorner = calculatedArea.corners[index];
                        const nextCorner = calculatedArea.corners[(index + 1) % calculatedArea.corners.length];
                        const deltaY = nextCorner.y - currentCorner.y;
                        const deltaX = nextCorner.x - currentCorner.x;
                        
                        return (
                          <tr key={index} className="hover:bg-gray-50">
                            <td className="border border-gray-800 px-2 py-1 text-center">{bd.bearingDMS}</td>
                            <td className="border border-gray-800 px-2 py-1 text-right">{bd.distance.toFixed(2)}</td>
                            <td className="border border-gray-800 px-2 py-1 text-center font-bold">{currentCorner.point}</td>
                            <td className="border border-gray-800 px-2 py-1 text-right">{currentCorner.y.toFixed(2)}</td>
                            <td className="border border-gray-800 px-2 py-1 text-right">{currentCorner.x.toFixed(2)}</td>
                            <td className="border border-gray-800 px-2 py-1 text-right">
                              {deltaY >= 0 ? '' : '-'}{Math.abs(deltaY).toFixed(3)}
                            </td>
                            <td className="border border-gray-800 px-2 py-1 text-right">
                              {deltaX >= 0 ? '' : '-'}{Math.abs(deltaX).toFixed(3)}
                            </td>
                          </tr>
                        );
                      })}
                ...
                <div className="mt-4 text-center">
                  <div className="inline-block border-t-2 border-gray-800 pt-2">
                    <span className="font-bold text-lg">
                      Area (Ha.) = {(calculatedArea.area / 10000).toFixed(4)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Summary Information */}
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <h5 className="font-semibold text-gray-900 mb-3">Survey Summary</h5>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 p-3 rounded-lg">
                    <div className="text-sm text-blue-800">Total Area</div>
                    <div className="text-lg font-bold text-blue-600">{calculatedArea.formattedArea.displayText}</div>
                  </div>
                  <div className="bg-green-50 p-3 rounded-lg">
                    <div className="text-sm text-green-800">Perimeter</div>
                    <div className="text-lg font-bold text-green-600">{calculatedArea.perimeter.toFixed(3)} m</div>
                  </div>
                  <div className="bg-purple-50 p-3 rounded-lg">
                    <div className="text-sm text-purple-800">Corners</div>
                    <div className="text-lg font-bold text-purple-600">{calculatedArea.corners.length}</div>
                  </div>
                  <div className="bg-yellow-50 p-3 rounded-lg">
                    <div className="text-sm text-yellow-800">Area (Hectares)</div>
                    <div className="text-lg font-bold text-yellow-600">{(calculatedArea.area / 10000).toFixed(4)} ha</div>
                  </div>
                </div>
              </div>
              ...
              {/* Coordinate Details */}
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <h5 className="font-semibold text-gray-900 mb-3">Coordinate Details</h5>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {calculatedArea.corners.map((corner: FieldObservation, index: number) => (
                    <div key={corner.point} className="grid grid-cols-4 gap-2 p-2 bg-gray-50 rounded text-xs">
                      <div className="font-bold text-center">
                        <div className="w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold mx-auto mb-1">
                          {index + 1}
                        </div>
                        {corner.point}
                      </div>
                      <div className="text-right">
                        ...
                        <div>{corner.y.toFixed(2)}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">X:</div>
                        <div>{corner.x.toFixed(2)}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xs text-gray-600">HRMS:</div>
                        <div>{corner.hrms.toFixed(3)}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Survey Quality */}
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <h5 className="font-semibold text-gray-900 mb-3">GPS Quality Metrics</h5>
                <div className="space-y-2 text-sm">
                  {(() => {
                    ...
                    const avgPDOP = calculatedArea.corners.reduce((sum: number, corner: FieldObservation) => sum + corner.pdop, 0) / calculatedArea.corners.length;
                    
                    return (
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-blue-50 p-2 rounded">
                          <div className="text-xs text-blue-800">Avg HRMS</div>
                          <div className="font-bold text-blue-600">{avgHRMS.toFixed(3)} m</div>
                        </div>
                        <div className="bg-green-50 p-2 rounded">
                          <div className="text-xs text-green-800">Avg VRMS</div>
                          <div className="font-bold text-green-600">{avgVRMS.toFixed(3)} m</div>
                        </div>
                        <div className="bg-purple-50 p-2 rounded">
                          <div className="text-xs text-purple-800">Avg Satellites</div>
                          <div className="font-bold text-purple-600">{avgSats.toFixed(0)}</div>
                        </div>
                        <div className="bg-yellow-50 p-2 rounded">
                          <div className="text-xs text-yellow-800">Avg PDOP</div>
                          <div className="font-bold text-yellow-600">{avgPDOP.toFixed(1)}</div>
                        </div>
                      </div>
                    );
                  })()}
