...
export const DigitalLodgment: React.FC = () => {
  ...
  const generateFieldBookPDF = async () => {
    ...
    yPosition += 8;
    pdf.text(`Instrument: ${fieldBookConfig.instrument}`, 20, yPosition);
    yPosition += 8;
    pdf.text(`Weather: ${fieldBookConfig.weather}`, 20, yPosition);
    yPosition += 20;

    // Field Notes Section (E1, E2, E3...)
    checkPageBreak(30);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text('FIELD NOTES - Page E1', 20, yPosition);
    yPosition += 15;
    ...
    if (fieldBookData.observations.length > 0) {
      const observationData = [
        ...
        ...fieldBookData.observations.map(obs => [
          obs.point,
          obs.y.toFixed(3),
          obs.x.toFixed(3),
          obs.hrms.toFixed(3),
